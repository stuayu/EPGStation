...
function createFakes() {
    ...
    const thumbnailReplaceRecordedCalls = [];
    ...
    const ipc = {
        ...
        thumbnail: {
            replaceRecorded: async (recordedId, videoFileId) => {
                thumbnailReplaceRecordedCalls.push({ recordedId, videoFileId });
            },
        },
    };
...
test('エンコード出力の登録後に既存サムネイルを新しい動画から差し替える', async () => {
    const fakes = createFakes();
    const model = createModel(fakes);
    model.set();
    ...
    assert.deepEqual(fakes.thumbnailReplaceRecordedCalls, [{ recordedId: 1, videoFileId: 100 }]);
});
...
test('fullOutputPath が null (ファイルサイズ更新のみ) の場合は addVideoFile と再解析を呼ばない', async () => {
    ...
    assert.equal(fakes.thumbnailReplaceRecordedCalls.length, 0);
});
