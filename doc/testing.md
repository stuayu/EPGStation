...
`video.currentTime` が進むことは映像が出ている証拠にならない。黒画面でも時刻だけ進む場合があるため、ブラウザシナリオは `video` 要素を canvas へ描画し、輝度の最大値で真っ黒フレームを、平均輝度のサンプル間差分で画面変化を判定する。`videoWidth === 0` や描画取得失敗も不合格とする。
