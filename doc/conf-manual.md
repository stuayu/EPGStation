...
- **この項目を設定すると、`cmd` を省略した配信プリセット (ライブ・録画 TS) の前段へ自動で tsreadex が入る**
  (`%TSREADEX% -x 18 -n -1 -a 13 -b 7 -c 5 -u 5 - | ...`)。設定しない場合は従来どおり tsreadex を通さない
  (実行ファイルを同梱していないため、無条件に挟むと配信が起動しなくなる環境があるので既定は無効)
- tsreadex は対象サービスの抽出・映像/音声 PID の固定・デュアルモノラル (二か国語) の主音声/副音声分離・
  欠落音声の補完を行う。**放送側で音声構成が変わっても配信は「映像 + 音声 2 本」の固定構造になる**ため、
  番組の切り替わりで再生が止まらない
- **`-b` は `7` (第 2 音声が無ければ第 1 音声をコピー) を使う。`5` (無音 AAC を挿入) にしない**。
  第 2 音声を選んだまま二か国語番組が終わる場合や、EPG が二か国語と言っていても実際の AAC が
  デュアルモノラルでない場合に、副音声が完全な無音になる (実測: `-b 5` で副音声 -91.0dB、
  `-b 7` で主音声と同じ -28.4dB。第 2 音声が実在する放送では両者に差は無い)
- tsreadex を通すと**デュアルモノラルは 2 本の音声 ES へ分離済み**になるため、副音声の選択は
  `-dual_mono_mode sub` ではなく `-map 0:a:1` で行う (`AudioTrackUtil` が cmd に `%TSREADEX%` が
  含まれるかで自動的に切り替える)
- ストリーミング設定の `cmd` 内の `%TSREADEX%` がこの値で置換される
- `cmd` に `|` を含めるとシェル経由で実行されるため、`cmd` を手書きする場合も
  `'%TSREADEX% -x 18 -n -1 -a 13 -b 7 -c 5 -u 5 - | %FFMPEG% ...'` のような前処理パイプラインが使える
...
配信時の音声ブースト倍率。既定値は `2.0`、`1.0`〜`4.0` の範囲に丸める。`1.0` では無効。
ライブ・録画再生の再エンコード配信だけに適用し、保存用 `encode` と音声コピーには適用しない。
...
ただし **`qualities` の中で一番解像度が高いものだけは HEVC でも 0.65 倍せず、H.264 と同じ
ビットレートを使う**。画質一覧の先頭は「帯域を使ってでも綺麗に見たい」ときに選ぶものなので、
削った帯域ではなく画質差として出すため。対象は `qualities` の内容で決まり、
`[2160p, 1080p]` なら 2160p が、`[1080p, 720p]` なら 1080p が該当する。

| quality | 解像度 | H.264 | HEVC | 音声 |
| ------- | ------ | ----- | ---- | ---- |
| 2160p   | 2160   | 24000 kbps | 15600 kbps | 256 kbps |
| 1080p   | 1080   | 8000 kbps  | 5200 kbps  | 256 kbps |
(HEVC 列は最高解像度でない場合の値。最高解像度に当たるものは H.264 列と同じになる)
