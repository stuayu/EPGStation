...
| allowAnonymous | boolean | no   | 未ログインでも一般ユーザーと同じ操作を許可するか。省略時 false            |
...
#### 未ログインでも使えるようにする (allowAnonymous)

`allowAnonymous: true` にすると、**ログインしていない利用者も一般ユーザーと同じ操作**
...
家庭内 LAN で「普段はログイン不要にしたいが、設定だけは保護したい」場合に使う。
...
- **インターネットに公開している場合は有効にしないこと。** 録画の閲覧・削除が誰でもできてしまう
