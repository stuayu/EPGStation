...
- CI は `.github/workflows/build-validation.yml` で 3 OS (ubuntu/windows/macos) × Node 18/20/22 を検証している。Node の対応下限を上げる場合はこのワークフローと `package.json` の `engines`、`Dockerfile.alpine` / `Dockerfile.debian` も更新する
