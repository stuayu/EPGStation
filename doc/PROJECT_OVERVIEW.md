...
| 録画番組のオフライン保存・再生 | `src/model/api/video/OfflineVideoApiModel.ts`, `src/model/service/api/videos/{videoFileId}/offline.ts`, `src/model/service/stream/llhls/OfflineFmp4RecordStream.ts`, `client/src/services/OfflineVideos.ts`, `client/src/views/OfflineVideos.vue`, `client/serviceWorker.template.js` | 録画 HLS プロファイルを `Fmp4Packager` で約6秒単位の fMP4 レコードへ逐次変換し、EPGODL2 の init/master/segment/end を Cache Storage / IndexedDB へ保存。保存経路は readrate・視聴用先行抑制を使わず、backpressure と自前 keep で EOF まで流す。複数音声レンディション、`CODECS` 付き master、字幕 `emsg`、ローカル HLS VOD (`VERSION:7` / `MAP`) に対応。Service Worker は Vite が index.html・assets・固定静的資産を build ごとの app cache へ precache し、navigation は network-first + index.html fallback、保存動画の `/local/offline/` は別 Cache Storage から返す。API・streamfiles・socket.io・データ放送/SNS WebSocket・サムネイル API は横取りしない。古い app cache だけ更新時に削除し、保存動画 cache は残す。オフライン起動時は接続系処理と API エラー通知を止め `/offline-videos` へ自動遷移する。録画中409、同時3件、切断時kill/枠解放。Original HEVC は保存対象、Original MPEG-2 / m2tsll は保存対象外 |
