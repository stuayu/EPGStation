...
- **録画音声一覧の ffprobe は遅延 ES を待つ**: `VideoUtil.getAudioTracks()` だけ `-analyzeduration 60000000 -probesize 200000000` (60 秒 / 200 MB) を指定する。本番の encoded HEVC TS では 2 本目の ES が主音声より 21.696 秒遅く始まり、既定 probe では 1 本、拡大後は 2 本を検出できた。`getDetailedInfo()` は映像の `field_order` / fps などの判定用なので変更せず、ライブの低遅延 probe 値とも分離する
