...
- **録画音声一覧は完了後にファイル全体を probe する**: `VideoUtil.getAudioTracks()` は完了録画では `-analyzeduration` / `-probesize` を int64 上限にして、途中で現れる ES も列挙する。録画中だけ `60 秒 / 200 MB` に制限し、まだ存在しない末尾を待たない。`getDetailedInfo()` とライブ配信の低遅延 probe 値は変更しない。音声 ES の数値指定は optional map と主音声 map を併記し、指定 ES が区間に無い場合も映像+主音声で配信を継続する。トラック一覧は全体候補なのでシークごとの再取得は行わない
