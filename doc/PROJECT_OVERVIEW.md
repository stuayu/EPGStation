...
- **tsreplace HEVC の Original は fMP4 無変換**: `SourceAnalyzer` が MPEG-TS の HEVC Main / Main10 と判定し、端末の `hevc` / `hevcMain10` 能力があるときだけ `original-hevc` を候補にする。サーバーは映像を copy し (音声は AAC へ再エンコード)、`-tag:v hvc1` を付けて in-memory HLS へ詰め替える。実AAC TSへ `aac_adtstoasc` は適用しない。MPEG-2 の `mpeg2toh264` 経路とは相互排他、「おまかせ」自動選択・fallback 対象外。ビット深度不明や unsupported codec は安全側で候補なし。録画詳細・視聴履歴・DPlayer・オフライン保存は playback-options の `modes` を共通の正とする。tsreplace / Amatsukaze 出力は `video_file.type=encoded` で登録されるため、字幕は `videoFileType` でなく `SourceAnalyzer.transport=mpegts` を条件に別readerから `emsg` 化する
