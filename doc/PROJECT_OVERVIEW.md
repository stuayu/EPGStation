...
- **tsreplace HEVC の Original は対応端末では元TSを直接配信する**: `SourceAnalyzer` が MPEG-TS の HEVC Main / Main10 と判定し、端末の `hevc` / `hevcMain10` 能力があるときだけ `original-hevc` を候補にする。対応端末では `/api/videos/{videoFileId}/original` が Range 付き `video/mp2t` を返し、mpegts.js の HEVC MSE/MMS へ渡すため、録画再生用の ffmpeg / tsreadex は起動しない。固定 tsukumijima fork は HEVC demux、`mseH265Playback`、音声切替 API を備える。MSE/MMS 非対応端末だけは従来の in-memory HLS (`hvc1` fMP4) へ fallback する。MPEG-2 の `mpeg2toh264` 経路とは相互排他、「おまかせ」自動選択には入れない。ビット深度不明や unsupported codec は安全側で候補なし。独立音声 ES 2本は mpegts.js の音声切替、デュアルモノラル1本は Web Audio API の左右振り分けを使う。録画詳細・視聴履歴・DPlayer・オフライン保存は playback-options の `modes` を共通の正とする。tsreplace / Amatsukaze 出力は `video_file.type=encoded` で登録されるため、字幕は `videoFileType` でなく `SourceAnalyzer.transport=mpegts` を条件に別readerから `emsg` 化する
