...
| オフライン保存動画の実況 | `client/src/util/JikkyoKakologClient.ts`, `src/util/OfflineJikkyoParam.ts`, `client/src/services/OfflineVideos.ts`, `client/src/views/OfflineVideoWatch.vue` | 保存時に `videoFile.startAt` 基準の実況チャンネル・取得範囲とチャプターを保存。オンライン再生時は保存値を優先し、旧形式は EPGStation API で解決して書き戻す。コメント取得失敗は再生を止めず、`online` 復帰時に未取得区間を再試行。Service Worker は外部過去ログ API を passthrough。オフライン視聴では VirtualTimeline の全体時間軸、チャプターマーカー、SNS (回線接続時) を使える |
...
| ストリーミング再生 | `src/model/stream/{capability,preset,resolver,builder}/`, `src/model/service/api/**/playback-options.ts`, `client/src/components/video/quality/`, `client/src/model/state/video/` | SourceAnalyzer → StreamPresetRegistry → PlaybackPolicyResolver → Live/RecordedCommandBuilder。Playback API 2 系統と端末能力対応の画質選択 UI。Playback API は preset id と container 別 mode を返し、DPlayer quality と配信選択ダイアログは同じ profile 一覧を使う。**再生中の画質切替は DPlayer の設定メニューだけが入口** (プレイヤー右上に別の設定アイコンを置かない)。**配信選択ダイアログの画質一覧はダイアログの中でインライン展開する** (別のモーダルを重ねない)。画質 → サーバ mode は `PlaybackProfile.modes[<container>]` で引く (プロファイル配列の添字を mode に流用しない)。DPlayer は `modes` が数値を持つ方式×画質だけを平坦な一覧へ出し、方式ラベルと画質ラベルを `PlaybackLabelUtil.ts` で「標準 (HLS) > 1080p 高画質」のように結合する。方式切替は同一 index の `switchQuality()` に依存せず、`VideoContainer` が対象コンポーネントとストリームを再生成し、録画の再生位置を引き継ぐ。**一覧へ出す方式は `VideoContainer` が prop (`selectablePlaybackContainers`) で各映像コンポーネントへ渡す** — 子コンポーネントはシーク・画質切替のたびに自分で `setPlaybackProfiles()` を呼び直すため、prop で持たせないと呼び直しのたびに一覧が現在の方式 1 つへ縮む (実測: 一覧 11 件 → 1 件)。**`setPlaybackProfiles()` の「現在位置」には quality 配列の添字ではなく実際のサーバ mode を渡す** (`currentMode` 引数)。添字を渡すと選択中の項目を取り違え、M2TS-LL で再生中なのに「MP4 > おまかせ」が選択中と表示される。 **同じ role のプリセットを複数持つ構成 (HEVC 版と AVC 版の M2TS-LL 720p など) では表示名が完全に一致する**ため、`disambiguatePlaybackLabels()` が重複したものだけへ `(HEVC)` / `(H.264)` を足す。判定材料の `PlaybackProfile.videoCodec` は `StreamPreset.output.codec` をそのまま API に載せたもの。**表示ラベルの引き当てキーは `PlaybackProfile.role`** (`auto` / `original` / `original-mpeg2` / `original-hevc` / `2160p-high` / `1080p-high` / `1080p` / `720p` / `data-saver`)。`profile.id` は `live-m2tsll-1080p-avc` のような実プリセット id なので、id で辞書を引くと `auto` 以外は必ず外れる (実際に一言説明とバッジが出ていなかった)。`role` はサーバが `PlaybackApiModel.builtinRole()` で決めて API に載せる。 **「おまかせ」プリセットを返すのはライブだけ**で、録画の配信では `profiles` に `auto` が入らない。`PlaybackOptionsState.getInitialPresetId()` は `auto` が無ければ `recommended.resolvedId` を初期選択にする (`auto` のままだと、一覧のどれも選択されていないのにボタンだけ「おまかせ」と出る)。 **画質の表示ラベル (名前・一言説明・技術詳細・バッジ) は `client/src/util/PlaybackLabelUtil.ts` の 1 か所で決まる** (`getPlaybackLabel()` / `getPlaybackShortLabel()`)。配信選択ダイアログの一覧と DPlayer 設定メニューの両方がここを経由するため、表記を変えるときはここだけを直す。設定 > 再生の既定値 (既定の画質 / 映像補正 / HDR / モバイル回線では画質を下げる) は `playback-options` の `profile` / `preferHdr` / `preferCorrection` / `saveData` として渡り、`PlaybackPolicyResolver` の自動選択に**加減点として**反映する (候補を絞り込まないので設定に合う候補が無くても再生できる)。既存 `stream:` は従来経路を維持。**DPlayer の quality 配列を更新するときは内部 `blob:` URL や空の `video.src` を採用せず、元の配信 URL (`options.video.url`) を優先する。mpegts.js 初期化 URL が空なら fail-fast する** |
| 録画番組のオフライン保存・再生 | `src/model/api/video/OfflineVideoApiModel.ts`, `src/model/service/api/videos/{videoFileId}/offline.ts`, `src/model/service/stream/llhls/OfflineFmp4RecordStream.ts`, `client/src/services/OfflineVideos.ts`, `client/src/views/OfflineVideos.vue`, `client/serviceWorker.template.js` | 録画 HLS プロファイルを `Fmp4Packager` で約6秒単位の fMP4 レコードへ逐次変換し、EPGODL2 の init/master/segment/end を Cache Storage / IndexedDB へ保存。MPEG-2 と非 iOS の HEVC Original は元 TS を188 byte境界・最大16MiBのチャンクで Cache Storage へ保存し、Service Worker が Range 要求をチャンクから組み立てて206/416で返す。iOS / iPadOS の `original-hevc` は `format=fmp4` で映像 copy (`hvc1`)・音声 AAC 再エンコードの fMP4 レコードへ切り替え、既存の `OfflineHLSVideo` でネイティブ HLS 再生する。`format=fmp4` は HEVC だけ受理し、`original-mpeg2` は変換対象外。Range 無し要求は保存ファイル全体を200で返し、`Content-Range` は付けない。**Range 無しの元 TS も Service Worker が `pull()` で読み手の要求ごとに少しずつ返し、保存チャンクを最大2MiBへ分割する。** **mpegts.js は TS VOD のキーフレーム index が無いと先読み範囲外へ seek できないため、オフライン original-hevc は `original.ts?offset=<188 byte境界>` を offset 後の独立ファイルとして返し、同じ DPlayer 上の mpegts.js を再生成する。** 保存時は番組情報、音声トラック一覧、局ロゴをスナップショットし、オフライン一覧から番組情報を表示する。保存経路は readrate・視聴用先行抑制を使わず、backpressure と自前 keep で EOF まで流す。複数音声レンディション、`CODECS` 付き master、字幕 `emsg`、ローカル HLS VOD (`VERSION:7` / `MAP`) に対応。Service Worker は Vite が index.html・assets・固定静的資産を build ごとの app cache へ precache し、navigation は network-first + index.html fallback、保存動画の `/local/offline/` は別 Cache Storage から返す。API・streamfiles・socket.io・データ放送/SNS WebSocket・サムネイル API は横取りしない。古い app cache だけ更新時に削除し、保存動画 cache は残す。保存済み元 TS のデータ放送は browser 内の `web-bml/worker` `decodeTS` を使い、保存 URL の Range fetch だけで再生する。オフライン起動時は接続系処理とAPIエラー通知を止め `/offline-videos` へ自動遷移し、画面を暗くするscrimは表示せずタイトルバーにオフライン表示を出す。録画中409、同時3件、切断時kill/枠解放。Original HEVC / MPEG-2 は保存対象、m2tsll は保存対象外 |
| 録画 M2TS-LL の再生成 | `client/src/components/video/RecordedStreamingVideo.vue`, `src/util/MpegTsLifecycleUtil.ts` | DPlayer 1.33.1 の `mediaBackendDestroy` が旧 mpegts.js 個体をクロージャで保持するため、同じ video 要素のレジューム・シークは新 URL 設定前に DPlayer の `destroyMediaBackend()` を1回だけ呼ぶ。自前の `mpegts.destroy()` や plugin 参照削除を先行させない。別要素を作る画質切替・ライブ再接続は旧 callback を退避して DPlayer の initMSE 内破棄を抑止し、旧 mpegts.js / ARIB renderer を `canplay` まで保持した後、旧 plugin 参照を復元して callback を1回だけ実行する。オリジナル MPEG-2 は DPlayer 標準の同じ callback で旧 `mpeg2toh264.destroy()` を1回実行する。**オフライン original-hevc は TS VOD のキーフレーム index 不在による範囲外 seek 不可を避け、Service Worker の offset URL で同じ video 要素上の mpegts.js を再生成する。** |
| データ放送 (BML) | `client/src/util/DataBroadcastingManager.ts`, `src/model/service/dataBroadcasting/` | `web-bml` (tsukumijima フォーク) を npm 依存で利用。iframe に隔離せず `BMLBrowser` を直接生成し、**映像要素を BML ブラウザの中へ物理的に移動**して DPlayer に組み込む。オンラインのライブ/録画は既存 WebSocket 経路を使い、保存済み `original-mpeg2` / `original-hevc` は `web-bml/worker` の `decodeTS` へローカル URL の188 byte境界 Range チャンクを投入するため通信断でも表示できる。録画は `videoFile.startAt + VirtualTimeline の絶対再生位置` を時計の単一基準にし、`startAt` が無い場合は推測時刻を送らない。`seeked` / `canplay` / 画質切替確定時へ即時反映する |
...
オフライン保存画面は `#/offline-videos` の一覧、`#/offline-videos/:key` の保存スナップショット詳細、`#/offline-videos/:key/watch` の視聴画面に分かれる。`key` は videoFileId + generationId の一意値。視聴画面は `WatchLayout` / `WatchTopBar` / `WatchSidePanel` の番組情報タブを基本にし、オンラインで実況パラメータがある場合はコメントタブも表示する。保存済みデータと Service Worker のローカル URL から描画し、`/api/recorded/**`・`/api/streams/**`・`/streamfiles/**`・視聴履歴 API は呼ばない。再生位置は localStorage へ保存・復元する。録画詳細・録画一覧の保存済みバッジと保存完了通知から視聴画面へ遷移する。視聴画面の戻る操作は履歴を優先し、直接起動時は `from` の起点に応じて一覧・番組情報・録画詳細へ戻す。

### オフライン保存視聴の制約

保存時のチャプターは `OfflineVideoRecord.chapters` に任意項目として保持し、旧レコードでは空配列として扱う。オフライン視聴は保存ファイルの全体長を使う `VirtualTimeline` でチャプターマーカーとシークを描画する。SNS 投稿パネルは回線接続時だけ表示し、データ放送は BML ES が保存される MPEG-2 Original かつ `/api/dataBroadcasting/ws` へ接続できる場合だけ表示する。HLS fMP4 は BML ES を含まないためデータ放送を表示しない。Next Up と画質切替は別録画のサーバー情報または複数保存プロファイルが必要なため対象外。
...
- **MPEG-2 TS の Original は端末デコード専用**: MPEG-2 TS のみ `original-mpeg2` を候補にし、ライブは tsreadex 正規化後の `video/mp2t`、録画は Range 付き元 TS を返す。`video_file.type=encoded` でも SourceAnalyzer が MPEG-2 MPEG-TS と判定すれば元TSを返す (MP4等の encoded は除外)。クライアントの libratechw `mpeg2toh264` `1e0eb60841daeacb1deae3def0192cf53512638e` が WASM / Worker / MSE で変換する。HEVC・非対応ブラウザー、「おまかせ」自動選択・停滞 fallback は対象外。録画シークでは `req.aborted` / `res.close` で ReadStream を destroy する。Safari 録画時は MediaSource を main thread へ置く。初回 autoplay は Safari / iOS 以外で有効にし、`VirtualTimeline` 経由のシークは再生中のときだけ `play()` で再開する
- **tsreplace HEVC の Original は対応端末では元TSを直接配信する**: `SourceAnalyzer` が MPEG-TS の HEVC Main / Main10 と判定し、端末の `hevc` / `hevcMain10` 能力があるときだけオンラインの `original-hevc` を候補にする。対応端末では `/api/videos/{videoFileId}/original` が Range 付き `video/mp2t` を返し、mpegts.js の HEVC MSE/MMS へ渡すため、録画再生用の ffmpeg / tsreadex は起動しない。固定 tsukumijima fork は HEVC demux、`mseH265Playback`、音声切替 API を備える。MSE/MMS 非対応端末だけは従来の in-memory HLS (`hvc1` fMP4) へ fallback する。MPEG-2 の `mpeg2toh264` 経路とは相互排他、「おまかせ」自動選択には入れない。ビット深度不明や unsupported codec は安全側で候補なし。独立音声 ES 2本は mpegts.js の音声切替、デュアルモノラル1本は Web Audio API の左右振り分けを使う。録画詳細・視聴履歴・DPlayer はこのオンライン判定を使う。オフライン保存の `original-hevc` は WebKit + 10bit を除外せず、mpegts.js の `mseH265Playback` だけで候補を判定する。実 Safari 26.6.2 で保存済み HEVC Main 10 / 1440x1080 を20秒再生し、`currentTime=19.75`、571 frames、dropped 0、輝度変化あり。tsreplace / Amatsukaze 出力は `video_file.type=encoded` で登録されるため、字幕は `videoFileType` でなく `SourceAnalyzer.transport=mpegts` を条件に別readerから `emsg` 化する
- **直接配信のレスポンスへ stream を pipe する API は配信中も keep する**: `result.stream.pipe(res)` の前後で `streamApiModel.keep()` を定期実行し、`res.close` / `finish` / stream 終了時にタイマーを止めて stream を停止する。これを欠くと `StreamBaseModel` の 15 秒タイマーでライブ Original 等が切れる。録画配信の `profile=auto` は `StreamApiModel.resolveProfile()` が mode を使って解決する
- **配信選択の profile id と mode を混同しない**: auto / original-hevc / 1080p が同じ mode を持ち得るため、ダイアログは `PlaybackProfileSelectUtil` で選択中 id と container 別 mode の一致だけを確認し、mode 番号から逆引きしない。視聴 URL に profile が指定されているときは `isRecordedWatchModeValid` が config の mode 一覧で弾かず、サーバーへ profile の解決を任せる
- **クライアント import 対象の util は Node API をトップレベルで実行しない**: `src/util/**` は `client/src/util/**` の再 export 経由で Web UI 全体へ bundle される。`Buffer` などはサーバー側の関数内でだけ参照し、import 時の `ReferenceError` で画面全体を白紙にしない
...
- **HLS は 2 モード**: cmd が `%streamFileDir%` を含まなければ in-memory 配信 (`HLSMemoryStoreModel`、ディスク書き込みなし)、含めば従来のディスク方式。ライブ・録画済みとも同じ判定で、**`encodePresets` が生成する HLS プリセットはどちらも in-memory (fMP4)**。**どちらのモードも ARIB 字幕対応**で、in-memory 側は ID3 を `emsg` box (**version 1 必須**) で運ぶ。encoded TS の `original-hevc` は字幕 reader の probe 中も主 stdout の `ftyp` / `moov` を保持する必要があるため、配信プロセスは `CreateProcessOption.drainStdout=false` とし、packager / offline record stream が stdout を読む。reader の `AribId3Extractor` 出力は drain して backpressure を防ぐ。in-memory HLS は最初の init / セグメントが15秒来ない場合に、既存の破棄動作を変えず warn ログを出す
...
- **音声トラックの切り替えは cmd のプレースホルダで行う**: `%DUALMONOMODE%` (`-dual_mono_mode main|sub`)、`%AUDIOMAP%` (`-map 0:v:0 -map 0:a:<n>`)、`%AUDIOSELECTMAP%` (既に映像 map がある cmd 用の `-map 0:a:<n>`)、`%AUDIOFILTER%` (pan / volume を 1 本の `-af` へ統合)。**二か国語放送のデュアルモノラルは `-map` では選べない** (1 つのステレオ ES の左右に主音声・副音声が入っているため)。TS 入力は `-dual_mono_mode sub`。encoded HEVC の副音声は、AACを main/sub へ実デコード比較して dual-mono と確認できた場合だけ `pan=stereo|c0=c1|c1=c1`、通常ステレオや判定不能は1本のステレオを維持する。EPG の componentType 0x02 や `channels=2` / `stereo` を実AACの証拠にしない。主音声へ pan は掛けない。ファイル直接再生は `audioTracks` が複数ならブラウザ実装、1 本のステレオ AAC なら Web Audio API で副音声を切り替える。Web Audio 非対応時は UI を出さず再生を維持する
...
- **`config.tsreadex` を設定すると、生成 cmd の前段へ tsreadex が入る** (`-x 18 -n -1 -a 13 -b 7 -c 5 -u 5`)。二か国語放送はデュアルモノラルで送られ、番組が切り替わると PMT の音声構成そのものが変わるが、tsreadex を通せば以降は「映像 + 音声 2 本 (PID 0x0110 / 0x0111 固定)」になる。**同梱していないため、設定が無い環境では従来どおり挟まない**。**`-b 5` (無音 AAC を挿入) にしないこと** — EPG が二か国語と言っていても実際の AAC がデュアルモノラルでない放送局があり、副音声が完全な無音になる (実測: `-b 5` で -91.0dB、`-b 7` で主音声と同じ -28.4dB)。**tsreadex を通すと副音声は `-dual_mono_mode sub` では選べない** (分離済みなので `-map 0:a:1`)。`AudioTrackUtil` が置換前の cmd に `%TSREADEX%` があるかで切り替える。**`audioTrack=all`** は tsreadex 正規化済み、または実音声 ES が 2 本以上のときに主音声・副音声の両方の ES (`-map 0:a:0 -map 0:a:1`) を同時に配信する (m2tsll でクライアントが再接続無しに切り替えるための経路。下記「再接続無しの音声切替」参照)。実音声 ES が 1 本または取得不明のときは `main` と同じ扱いにする。tsreadex 正規化済みで `audioTrack` 未指定なら index 0 (主音声 ES) を明示的に選ぶ (未指定のまま空にすると m2tsll では何も map されず配信が始まらない)
- **複数音声 ES を map する録画配信は probe を広げる**: 放送の副音声 ES は番組の途中から現れることがあり (実測: 主音声の 20MB あと)、パイプ入力の ffmpeg は probe 終了時点の PMT で map を確定する。既定の `-probesize 5000000` では 2 本目が「入力に存在しない」扱いになり、`-map "0:a:1?"` は optional なので黙って無視されて主音声だけが配信される (副音声へ切り替えられない)。`AudioTrackUtil.ensureMultiAudioProbe()` が `0:a:1` を含む cmd の `-probesize` / `-analyzeduration` を 50MB / 30 秒まで引き上げる。適用は録画配信で実 ES 2 本以上かつ tsreadex 未経由のときだけ (tsreadex 経由は先頭から 2 本に正規化済み、ライブは実時間供給なので probe を広げると初期遅延になる)
- **tsreadex 未経由の複数音声 ES も音声切替対象**: 録画は `VideoUtil.getAudioTracks()`、ライブは番組情報 `audios[]` から実 ES 数を求め、2 本以上なら `sub` を `-map "0:a:1?"` で選ぶ。1 本または取得失敗時は従来の `-dual_mono_mode sub` へ戻す。複数 ES の map は、in-memory HLS / embedded 切替可能な m2tsll では主音声を先に置いて `audio0` / `audio1` と役割を一致させ、MP4 / WebM / M2TS / 切替不可 m2tsll では選択 ES を先に置いてブラウザの 1 本目を副音声にする。複数レンディションを作る条件と `embeddedAudioSwitch.hls=true` の条件も、tsreadex 済みまたは実 ES 2 本以上、かつ in-memory HLS に統一する。rigaya の HLS/MP4 cmd は `%AUDIOMAP%` が映像 map を含むため、別の `-map 0:v:0` を出さない
...
`PlaybackProfile.embeddedAudioSwitch` (コンテナ別 boolean) が「主音声・副音声を再接続無しで同時配信できるか」を示す。`PlaybackApiModel` は m2tsll の実プロファイルに音声選択用プレースホルダ (`%AUDIOMAP%` / `%AUDIOSELECTMAP%`) があり、tsreadex 済みまたは実音声 ES が 2 本以上のとき true にする (デュアルモノラルの 1 ES は false)。hls は音声 map プレースホルダを含み、tsreadex 済みまたは実音声 ES が 2 本以上で、in-memory HLS (cmd が `%streamFileDir%` を含まない) のとき true になる (`Fmp4Packager` が音声 trak 2 本を検出して映像・音声を別レンディションへ分解し、`#EXT-X-MEDIA` 付きのマスタープレイリストを返す。**マスターの `CODECS` は必須**で、無いと Safari のネイティブ HLS が再生できない。**複数音声のレンディションは LL-HLS にしない** — パート付きだと Safari が音声を先頭セグメントまでしか取得せず再生が止まる)。Original HEVC は `original` も true になり、元TSを `audioTrack=all` で開いて独立 ES を mpegts.js の `switchPrimaryAudio()`/`switchSecondaryAudio()` で切り替える。デュアルモノラル1 ES は同 APIでは分離できないため Web Audio API で左右を切り替える。HLS のクライアント側切替は hls.js の `hls.audioTrack` / ネイティブ HLS の `video.audioTracks[i].enabled` (`client/src/util/HlsAudioTrackUtil.ts`)。クライアント (`LiveMpegTsVideo.vue` / `RecordedStreamingVideo.vue`) は `embeddedAudioSwitch.m2tsll === true` のとき `audioTrack=all` で開き、音声切替を `dp.plugins.mpegts.switchPrimaryAudio()`/`switchSecondaryAudio()` の直接呼び出しで行う (再接続しない)。画質切替で mpegts.js インスタンスが作り直された直後 (新インスタンスは常に主音声から始まる) は副音声の選択を再適用する。false/不明なら `switchQuality()` へ委譲せず、m2tsll は `audioTrack` 付き URL を `switchVideo()` へ渡し、HLS は `stop()` → `start()` → URL 差し替えで現在位置から再接続する
- **オフライン original-hevc の独立音声 ES 切替**: 既に append 済みの主音声は mpegts.js の `switchPrimaryAudio()` / `switchSecondaryAudio()` だけでは置き換わらないため、`mediaDataSource.url` に保存済み `videoSrc` を明示した DPlayer の `switchVideo()` で media backend を破棄し、現在位置から offset URL で読み直す。同じ video 要素の reset は DPlayer の `destroyMediaBackend()` を URL 設定前に1回だけ通し、新個体は `deferLoadAfterSourceOpen=false` で load 時に transmuxer を同期生成する。生成直後かつ PMT 解析前に音声選択を適用し、切替後は VirtualTimeline の絶対位置を復元する。DPlayer の `mediaBackendDestroy` を外から先行実行せず、字幕・データ放送・実況の同じ video 要素を維持する。失敗時は理由を snackbar に出し、選択表示を戻して元の再生を再生成する。再生中は再開し、一時停止中は停止を維持する。デュアルモノラルは Web Audio のまま読み直さない
...
- **`mpegts.js` は tsukumijima フォークをコミット SHA で固定する** (`client/package.json`)。本家 npm 版には Safari 向けの実運用修正が入っていない (音声タイムスタンプのギャップを埋める処理が Safari だけ無効化されており、タイムラインが縮んで **Safari 26.5 以降で再生が止まる**。ライブの `MediaSource.duration` も `Infinity` にならない)。ブランチ参照にすると lockfile の integrity が壊れて CI が落ちる。さらに build 時は `client/mpegtsAdtsPatch.js` の置換 (ADTS 偽同期対策と、TSDemuxer が生成時に `config.preferSecondaryAudio` を読む副音声の初期指定) を `client/vite.config.ts` から dist へ適用する。依存更新時は置換元が1箇所存在することを確認し、見つからない場合は build を失敗させる
