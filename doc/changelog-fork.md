...
- **録画の HLS 再生が 1〜2 分で止まったまま戻らなくなるのを直した (エンコード抑制のデッドロック)**
    ...
    - **対処**: 抑制を「完全停止」から「実時間ペースまで落とす」に変えた
      (`RecordedStreamBaseModel.throttleEncodeIfTooFarAhead()`)。
      `MAX_AHEAD_SEGMENT_NUM` (60) を超えたら標準出力の読み出しを `PACE_INTERVAL` (1 秒) だけ止め、
      **先行量が減っていなくても必ず再開する**。1 セグメント (約 1 秒) 生成するたびに 1 秒止まるので、
      エンコードは実時間相当のペースで進み続け、プレイリストも更新され続ける。
      完全停止は `HARD_MAX_AHEAD_SEGMENT_NUM` (120) を超えた場合だけに残した
      (プレイヤーが一時停止・離脱して取得自体をやめている状況の保険。この場合は
      0.5 秒間隔で確認し、先行量が 120 以下へ戻ったら再開する)
    - **注意点**: 録画済み in-memory HLS では**エンコードを完全に止めてはいけない**。
      止めるとプレイリストが更新されず、プレイヤー側から見て「サーバーが停止した」状態になる。
      先行を抑えたいときは「止める」のではなく「遅くする」こと。
      なお `HARD_MAX_AHEAD_SEGMENT_NUM` は保持数 (`RECORDED_RETAIN_SEGMENT_NUM` = 180) より
      十分小さい値である必要がある (超えると再生位置のセグメントが押し出される)
    - **検証**: `test/ut/recorded-hls-memory-stream.test.js` (ペーシングでの自動再開・
      HARD_MAX 超過時の停止継続・抑制不要時の 3 ケース)
