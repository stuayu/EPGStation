...
- HEVC Original の音声は `GET /api/videos/{videoFileId}/audio-tracks` の実 probe 結果を使う。独立した音声 ES が2本なら mpegts.js の `switchPrimaryAudio()` / `switchSecondaryAudio()` で再接続せず切り替える。1本の ES がデュアルモノラルなら、クライアントの Web Audio API で左右を主音声 / 副音声へ振り分ける。AAC の `channels` や EPG の componentType だけでは判定しない。MPEG-2 Original の `mpeg2toh264` 経路とライブ配信の挙動は変更しない。
