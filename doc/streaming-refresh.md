...
- HEVC Original の候補は Main / Main10 の端末能力に応じて表示し、自動選択・自動 fallback には入れず明示選択だけとする。直接 MPEG-TS の Range 応答は MPEG-2 Original と共通の `responseFile` を使い、MP4 等の非TS encoded は対象外とする。
