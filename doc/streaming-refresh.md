...
- tsreplace の HEVC TS (`stream_type=0x24`) は `video_file.type=encoded` で登録されても、`original-hevc` として Original 配信方式の候補へ表示する。対応端末で選択した実体は `GET /api/videos/{videoFileId}/original` の `video/mp2t` Range 応答を `mpegts.js` の MSE/MMS へ直接渡す経路であり、録画再生のためにサーバーで ffmpeg / tsreadex を起動しない。判定は拡張子でなく `SourceAnalyzer.transport === 'mpegts'` と映像 codec `hevc` を使う。固定 SHA (`bf4e49d0ff004cf7546237393a62c66e4b926dca`) の tsukumijima mpegts.js fork は `TsDemuxer` の HEVC NAL / `HEVCDecoderConfigurationRecord` 処理、`features.js` の `mseH265Playback` 判定、`Transmuxer.switchPrimaryAudio()` / `switchSecondaryAudio()` を持つため、この経路で利用する。MSE/MMS の HEVC 非対応端末は既存の `original-hevc` HLS fallback (`Fmp4Packager` / `HLSMemoryStoreModel`) へ切り替える。
- HEVC Original の音声は `GET /api/videos/{videoFileId}/audio-tracks` の実 probe 結果を使う。独立した音声 ES が2本なら mpegts.js の `switchPrimaryAudio()` / `switchSecondaryAudio()` で切り替える。オンラインの同時配信 m2tsll は再接続しないが、オフライン `original-hevc` は `mediaDataSource.url` に保存済み `videoSrc` を明示し、既に append 済みの主音声を捨てるため DPlayer の `switchVideo()` に旧 media backend の破棄・offset URL からの読み直しを任せる。同じ video 要素の reset は DPlayer の `destroyMediaBackend()` を URL 設定前に1回だけ通し、新個体は `deferLoadAfterSourceOpen=false` で load 時に transmuxer を同期生成する。生成直後かつ PMT 解析前に音声選択を適用する。切替失敗時は理由を snackbar に表示し、元の URL・位置・速度・音声で再生を再生成する。1本の ES がデュアルモノラルなら、クライアントの Web Audio API で左右を主音声 / 副音声へ振り分け、読み直さない。AAC の `channels` や EPG の componentType だけでは判定しない。MPEG-2 Original の `mpeg2toh264` 経路とライブ配信の挙動は変更しない。
- **オフライン original-hevc の範囲外 seek**: tsukumijima mpegts.js は TS VOD の `hasKeyframesIndex=false` では `seek()` が何もしない。先読み範囲内は `video.buffered` を確認して通常 seek、範囲外・初回 `playPosition`・独立音声切替は `original.ts?offset=<188 byte境界>` で同じ video 要素上の mpegts.js を再生成する。Service Worker は offset 後を独立ファイルとして扱い、`Content-Length` / `Content-Range` / 206 / 416 は offset 後の論理サイズ・位置で返す一方、Cache Storage のチャンク参照は保存ファイルの絶対位置で行う。`pull()` と最大2MiB分割は維持する。再生成時は `basePlayPosition + video.currentTime` を絶対位置とし、VirtualTimeline、チャプター、実況、データ放送、視聴位置保存を復元する。`deferLoadAfterSourceOpen=false` で load 時に transmuxer を同期生成し、PMT 解析前に音声トラックを適用する。再生中は再開、一時停止中は停止を維持する。
- オンラインの HEVC Original の候補は Main / Main10 の端末能力に応じて表示し、自動選択・自動 fallback には入れず明示選択だけとする。直接 MPEG-TS の Range 応答は MPEG-2 Original と共通の `responseFile` を使い、MP4 等の非TS encoded は対象外とする。オフライン保存の `original-hevc` は WebKit + 10bit を除外せず、mpegts.js の `mseH265Playback` だけで候補を判定する。実 Safari 26.6.2 で HEVC Main 10 / 1440x1080 の保存済み動画を20秒再生し、`currentTime=19.75`、571 frames、dropped 0、輝度変化ありを確認した。
...
- encoded TS の HLS / Offline では、拡張子でなく `SourceAnalyzer.transport === 'mpegts'` を見て字幕専用 reader を起動する。reader の `-ss` は VBR byte seek を使わず、主映像と同じ ffmpeg copy probe で得た実開始位置へ合わせる。reader の相対PTSは最初の映像partの `tfdt` を基準に `Fmp4Packager` が該当partへ分配するため、字幕readerが全尺を先に読み終えても先頭へ一括付与しない。副音声 (`%AUDIOMAP%`)・BML時計・実況同期の基準 (`videoFile.startAt`) は既存の主配信経路を維持する。`original-hevc` の複数音声を Offline 保存するときは、実 probe が `multi` / `dual-mono` の encoded TS だけファイルを tsreadex (`-x 18 -n -1 -a 13 -b 7 -c 5 -u 5`) へ流してから ffmpeg の `pipe:0` へ渡す。単一音声は `-b 7` で音声 ES が複製されるため tsreadex を通さず、保存済み master に副音声を後付けしない。オンライン HLS / m2tsll / Original 直接配信は従来どおりシーク可能な file / pipe 入力を使う。複数音声はオンライン HLS / Offline のどちらも `video` / `audio0` / `audio1` の 3 レコードへ分解し、master の `#EXT-X-MEDIA` と `CODECS` を生成する。`PlaybackProfile.embeddedAudioSwitch.hls=true` のとき、クライアントは `audioTrack=all` で開き、再接続なしに rendition を切り替える。
...
保存形式は `EPGODL2`、長さ付き JSON メタデータ、init/master/segment/end の型付きレコード。連続する fMP4 フラグメントを約6秒 (既定12 parts) ずつまとめ、init は別レコードとして先頭に送る。複数音声は `Fmp4Packager` の映像・音声レンディション、各 init、`CODECS` 付きマスタープレイリストを保存し、LL-HLS のタグは公開しない。ARIB 字幕の `emsg` は各フラグメント先頭のまま保存する。クライアントはレコード完成ごとに進捗を更新し、Cache Storage へ1件ずつ書く。保存後に `#EXT-X-VERSION:7` / `#EXT-X-MAP` / `#EXT-X-ENDLIST` のローカル fMP4 VOD を生成し、iOS Safari のネイティブ HLS と hls.js の両方で再生する (HEVC は `hvc1`)。保存画質は HEVC TS の `original-hevc` と MPEG-2 TS の `original-mpeg2` を許可する。MPEG-2 Original は元 TS を188 byte境界の最大16MiBチャンクへ分割して保存し、Service Worker が `/local/offline/.../original.ts` の Range をチャンクから組み立てる。Range 無し要求は全体を `200` で返し、`Content-Range` は付けない。範囲外は `416` と `Content-Range: bytes */<size>` を返す。再生はオンラインの `mpeg2toh264` 経路を使う。

保存済み HEVC Original は、MPEG-2 の `mpeg2toh264` とは異なり、元 TS を mpegts.js へ直接渡す。`original.ts?offset=<bytes>` は offset 後を独立した TS ファイルとして返し、Range の論理位置を offset 後へ揃える。offset は `floor(position * originalFileSize / durationSeconds / 188) * 188` とし、`0` から `fileSize - 188` へクランプする。これにより mpegts.js の先読み範囲を超えるシーク・位置復元・音声切替でも、同じ video 要素上で新しい個体を生成できる。

オフライン HLS の主音声・副音声は保存時の `#EXT-X-MEDIA:TYPE=AUDIO` と `AUDIO="audio"` を正とする。オフライン視聴画面はローカル master を読み、DPlayer 標準の音声メニューへ rendition 一覧を接続する。hls.js では `hls.audioTrack`、native HLS では `video.audioTracks[i].enabled` を切り替える。複数音声のローカル playlist は通常の VOD (`VERSION:7`、`ENDLIST`) とし、LL-HLS の `PART` / `PRELOAD-HINT` を付けない。保存済み単一音声データに後から副音声を追加することはできないため、音声切替には再保存が必要。

保存開始時は録画ファイル先頭の実時刻 (`videoFile.startAt`) を基準にニコニコ実況のチャンネル ID・取得範囲を解決し、保存レコードへ任意項目として記録する。旧形式レコードに値が無い場合は、再生時に `navigator.onLine !== false` かつ EPGStation サーバへ到達できるときだけ解決して書き戻す。オンラインのオフライン視聴は通常録画視聴と同じ `RecordedJikkyoSync` を使い、弾幕と「コメント」タブを同期する。過去ログ API の失敗は再生を止めず、取得済みコメントを保持したまま `online` イベントで未取得区間を再試行する。Service Worker は外部の過去ログ API を passthrough し、キャッシュしない。

端末能力判定は、HLS を hls.js なしで再生する iPhone / iPad / Safari では `decodingInfo({ type: 'file' })` と `canPlayType`、MSE を使うブラウザーでは `decodingInfo({ type: 'media-source' })` を使う。HEVC Main10 は10bit SDR の codec 判定であり、HDR/HLG 表示能力とは別に判定する。判定結果の localStorage キーは `epgstation.playback.client-capabilities.v3`。設定 > 再生の「この端末の再生能力」または `capability-report` ハーネスで確認できる。

クライアントはビットレートと録画時間から見積もった容量を `navigator.storage.estimate()` で確認し `navigator.storage.persist()` を要求する。MPEG-2 Original は元ファイルサイズで空き容量を判定する。バイナリパーサーは任意のチャンク分割を許容し、1レコード分だけを保持してマジック・メタデータ・role別連番・長さ・終端件数・EOF を検証する。保存本体は Cache Storage、番組情報・画質・世代・容量・局ロゴは IndexedDB / Cache Storage。Service Worker は Vite ビルド時に index.html・ハッシュ付き assets・`public/` 固定資産を app cache へ precache する。navigation は network-first + index.html fallback、静的資産は cache-first。`/local/offline/` は保存用 Cache Storage から返し、API、`/streamfiles/`、socket.io、データ放送/SNS WebSocket、サムネイル API には介入しない。app cache の世代更新で保存用 cache は削除しない。config 取得に失敗したオフライン起動は接続系処理と通知を止め、`/offline-videos` を表示する。画面を覆う切断scrimは使わず、タイトルバーのオフラインchipだけを出す。録画一覧の保存済み判定はIndexedDBを一度だけ読み、保存済み動画の再生は `#/offline-videos/:key/watch` へ遷移して行う。

### iOS / iPadOS・Android の保存制約
...
macOS Safari 以外 (iOS / iPadOS、Android や PC の Chrome など) の保存ダイアログで `original-hevc` を選んだ場合は、`GET /api/videos/{videoFileId}/offline?profile=original-hevc&format=fmp4&audioTrack=all` を使う。サーバーはオンラインの HLS > オリジナルと同じ `createOriginalHevcHlsCommand()`、`RecordedStreamBaseModel.startOfflineFmp4Packaging()`、`OfflineFmp4RecordStream` を通し、映像を `-c:v copy -tag:v hvc1`、音声を AAC へ再エンコードし、複数音声 rendition と字幕 `emsg` を含む EPGODL2 fMP4 レコードを返す。クライアントは `kind: 'hls'` として Cache Storage へ保存し、`OfflineHLSVideo` の `type: 'hls'` で再生する (Safari はネイティブ HLS、Chrome は hls.js)。元 TS を mpegts.js で再生する形式は、iOS / iPadOS では MSE が 10bit HEVC 60fps を実時間デコードできず (録画 23023 の 60fps 区間で 40 秒間に再生位置 4.55 秒)、Chrome では放送音声の壊れた AAC フレームで `PIPELINE_ERROR_DECODE` になり 0:00 / 10:00 / 20:00 から再生して 3.8 / 39.0 / 23.6 秒で停止した (fMP4 は各 3 分でエラー 0)。保存サイズ見積もりは元ファイルサイズ程度にする。`format` 省略時の original-hevc 元 TS Range 保存は macOS Safari だけで使い (データ放送のオフライン表示も元 TS が必要)、original-mpeg2 は全端末で元 TS のまま。`format=fmp4` は original-hevc だけ受理する。
...
tsreplace の HEVC 録画では壊れた AAC ES 内に `0xFFF` の偽同期語が混じることがある。mpegts.js の dist は12bit同期語だけでフレーム先頭と判定するため、偽ヘッダを音声設定変更として扱い、Safari の SourceBuffer を壊す。build 時に `client/mpegtsAdtsPatch.js` を `client/vite.config.ts` の transform から適用し、layer、sampling frequency index、frame length、次フレームヘッダを検証する。次ヘッダが入力末尾から4 byte未満なら検証を省略して、PES末尾の正規フレームを保持する。dev の optimizeDeps では esbuild plugin (`esbuildOptions.plugins` の `onLoad`) から同じ置換を適用する。Vite 8 の Rolldown最適化でもトップレベルの同じ transform が適用される。置換元が1箇所でない場合は build を失敗させるため、mpegts.js の SHA を更新するときは対象文字列の存在を確認する。
...
in-memory HLS はパッケージング開始から15秒たっても最初の init またはセグメントが来なければ、ストリームを破棄する既存動作を変えずに warn ログを出す。ライブで rigaya 系エンコーダを手書きする場合は `--input-analyze` / `--input-probesize`、ffmpeg は `-analyzeduration` / `-probesize` を小さくし、probe 中に出力が止まる設定を避ける。**セグメントが 1 本もできないまま破棄される事象は probe 設定だけが原因とは限らない** (実測: `--input-analyze 20 --input-probesize 50000000` を 1 秒 / 2MB へ縮めても解消せず、パイプ最上流の tsreadex まで CPU 0 秒 = TS が届いていない状態だった環境がある)。警告が出たら cmd を手で実行し、tsreadex / エンコーダ / ffmpeg のどこで止まっているかを切り分ける。
