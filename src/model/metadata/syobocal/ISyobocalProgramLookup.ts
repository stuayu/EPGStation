export interface SyobocalProgramMatch {
    ...
    exactStart: boolean;
    // 放送予定 (ProgItem.Flag) が再放送を示している場合 true。
    // 「その回が再放送として編成されている」という外部の事実なので、録画タイトルの "(再)" 表記や
    // ローカル DB の重複判定より確度が高い (放送種別の判定に最優先で使う)
    isRerun: boolean;
    // 放送予定が新番組 (初回) を示している場合 true
    isFirstEpisode: boolean;
    // 放送予定が最終回を示している場合 true
