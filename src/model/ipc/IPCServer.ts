...
export default class IPCServer implements IIPCServer {
     ...
     */
    public notifyProgramUpdatedClient(option: {
        channelIds: number[];
        startAt: number | null;
        endAt: number | null;
    }): void {
        ...
        this.child.send(<any>(<NotifyProgramUpdatedMessage>{
    ...
    private getThumbnailFunctions(): IFunctionIndex {
        ...
        index[ThumbnailFunctions.regenerateRecorded] = async msg => {
            const recordedId = this.getArgsValue<apid.RecordedId>(msg, 'recordedId');
            const profile = this.getArgsValue<'fast' | 'balanced' | 'quality' | undefined>(msg, 'profile');
