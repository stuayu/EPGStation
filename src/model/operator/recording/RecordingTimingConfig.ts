...
/** 既定の張り付き時間 (ms) */
export const DEFAULT_PREP_MS = 15 * 1000;
...
export const resolveRecordingTimingConfig = (
    ...
    const startMarginMs = Math.max(
        toMs(source.startMarginSec, 0),
        toMs(timeSpecifiedStartMarginSec, DEFAULT_RECORDING_TIMING_CONFIG.startMarginMs / 1000),
    );
    const endMarginMs = Math.max(
        toMs(source.endMarginSec, 0),
        toMs(timeSpecifiedEndMarginSec, DEFAULT_RECORDING_TIMING_CONFIG.endMarginMs / 1000),
    );
