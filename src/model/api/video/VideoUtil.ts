...
export default class VideoUtil implements IVideoUtil {
    ...
    // 本番の encoded TS で約 21.7 秒遅れて始まる ES を検出できた実測値 (ffprobe の単位はμ秒 / byte)。
    private static readonly AUDIO_TRACK_FFPROBE_ANALYZE_DURATION = 10 * 1000 * 1000;
    private static readonly AUDIO_TRACK_FFPROBE_SIZE = 20 * 1000 * 1000;
    // 完了済み・録画中とも同じ有限上限を使う。完了後もファイル全体を走査せず、録画中も末尾を待たない。
    private static readonly COMPLETE_AUDIO_TRACK_FFPROBE_LIMIT = {
    ...
    private static readonly RECORDING_AUDIO_TRACK_FFPROBE_LIMIT = {
        analyzeduration: VideoUtil.AUDIO_TRACK_FFPROBE_ANALYZE_DURATION.toString(10),
        probesize: VideoUtil.AUDIO_TRACK_FFPROBE_SIZE.toString(10),
    };
