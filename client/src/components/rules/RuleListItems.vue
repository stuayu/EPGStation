            ...
            <v-list-item v-for="item in items" v-bind:key="item.display.id" v-bind:class="{ 'selected-color': item.isSelected === true }">
                <div class="v-list-item-content">
                    <div class="pl-2 d-flex flex-row flex-wrap align-center" v-on:click="selectItem(item)">
                        <v-switch color="primary" v-if="isEditMode === false" v-model="item.display.isEnable" hide-details density="compact" v-on:update:model-value="changeState(item)" class="toggle"></v-switch>
                        <div class="keyword">
                            <!--
                            {{ item.keyword }}
                            -->
