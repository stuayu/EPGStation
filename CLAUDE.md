...
- **`mpegts.js` は tsukumijima フォークをコミット SHA で固定**。本家 npm 版は Safari で音声タイムスタンプのギャップ補完が無効化されており、Safari 26.5 以降で再生が止まる。build 時は `client/mpegtsAdtsPatch.js` の ADTS 偽同期対策を `client/vite.config.ts` から適用し、依存更新時は置換元が1箇所存在することを確認する。見つからない場合は build を失敗させる
