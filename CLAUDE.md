...
| `sweep-similar-bugs` | 不具合を直したあと、同じ型の不具合が他に無いか洗い出す |
...
**個別タスクで毎回書かなくてよいように、ここに常設する。** タスク側の指示がここと矛盾する場合はタスク側を優先する。

### 承認なしで進めてよいこと

- **修正 → ビルド → テスト → 再修正のループ**。途中で伺いを立てず、通るまで進める
- `npm run compile` / `npm test` / `npm run lint` / `cd client && npm run build` の実行
- 調査用の使い捨てスクリプトを**リポジトリ外の作業ディレクトリ**へ置いて実行する
- 既存テストの期待値の更新 (実装を正とする根拠を報告に書くこと)

### 触ってはいけないもの

- **`config/config.yml`** — 利用者所有の実環境設定 (git 管理外)。テンプレート (`config/*.yml.template`) は可
- **`git commit` / `git push`** — 指示役が行う。作業ツリーは変更したまま残す
- **動作中のサーバの起動・停止** (既定 port 8888) — 実機検証は指示役が行う
- **GitHub の Issue / PR へのコメント・close** — 指示役が行う
- `data/` 配下の実データ、`logs/`、`recorded/` の実録画ファイル

### 完了条件 (ここまでやって「完了」)

**最初の実装で止めない。** 次がすべて揃って完了とする。

1. `npm run compile` が通る
2. `npm test` が通る (UT + ITA)
3. **`npm run lint` が通る** — `npm run build` は eslint を含むため、これを飛ばすと本番ビルドが失敗する。`compile` と `test` は lint を通らないので別に実行する
4. `cd client && npm run build` が通る (クライアントを触った場合)
5. 変更に対応するテストを追加・更新した
6. 関連ドキュメントを更新した (上の「ドキュメント更新ルール」)

### 報告に必ず書くこと

- **「直った」と書くときは、何を測ってそう言えるのかを書く**。単体テストが通ったことは「利用者の症状が直った」根拠にならない
- 実測値は数値で書く (件数・秒・倍速・バイト数など)。**修正前後の両方**を出す
- 検証していない部分は「未検証」と明示する
- 指示役が実機で確認するためのコマンドを、そのまま貼れる形で書く

### 実機での測り方

配信・再生の不具合は `tools/playback-harness/` のシナリオで測れる (`node tools/playback-harness/run.js --help`)。
新しい測り方を作ったら、書き捨てずにここへシナリオとして足す。
...
- 報告は鵜呑みにせず、**重要な結論は指示役が実機で測って確かめてから統合する**
...
- **mpegts.js は TS VOD のキーフレーム index が無いと先読み範囲外へ seek できない**。オフライン original-hevc は Service Worker の `original.ts?offset=<byte>` で offset 後を独立ファイルとして返し、同じ DPlayer 上の mpegts.js を再生成する
...
- **録画済み HLS はエンコードを再生位置の近くに留める**。エンコードは実時間の数倍速なので、放置すると再生位置との差が際限なく開く。`RecordedStreamBaseModel` が先行量 (`HLSMemoryStoreModel.getAheadSegmentNum()`) を見て、150 セグメント (`MAX_AHEAD_SEGMENT_NUM`) を超えたらエンコーダの stdout の読み出しを止める。保持側 (`HLSMemoryStoreModel`) は「最新から一定件数」ではなく**再生位置 (`lastServedSeq`) から遡って約 120 秒分** (`RECORDED_KEEP_BEHIND_SEGMENT_NUM`) を基準に古いセグメントを破棄する — Safari のネイティブ HLS は再生位置から約 50〜60 秒先まで取得・バッファするため、「最新から件数」で保持窓を決めると先行しすぎたぶん窓の先頭が再生位置を追い越し、hls.js / ネイティブ HLS がライブエッジへ強制シークして再生位置が飛ぶ (実測: 数分再生すると発生)。**エンコードの抑制は完全には止めない** — 止めるとプレイリストの更新も止まり、ブロッキング要求の応答が変わるまで次を取りに来ないプレイヤー (iOS Safari 等) が飢えて先行量も減らず、再生が止まったまま戻らなくなる。抑制は超過量に比例した停止 ((先行量 - 150) × 100ms、上限 5 秒 = `pauseTime`) で行い必ず再開する。**再開判定のループ上限は固定の 5 秒ではなく `pauseTime` を使う** — 固定 5 秒にすると、超過がわずかで `pauseTime` が短く計算された場合でも常に 5 秒まで停止が引き延ばされる (先行量は視聴の実時間経過でしか減らないため)。**粗い ON/OFF にもしない** (再開時にバーストして再生がとびとびになる)。**エンコーダの正常終了 (exit code 0) はストリームを止めない** — 録画のエンコードは再生が終わるより先に必ず終わるため、終了をそのまま停止に結び付けるとまだ再生していない末尾セグメントを失う。`HLSMemoryStoreModel.markEnded()` でプレイリストへ `#EXT-X-ENDLIST` を付けるだけに留め、実際の停止はクライアント切断や keep タイマー切れに任せる (異常終了は従来どおり即座に停止する)
...
- **LL-HLS にするのはライブの単一音声だけ**。録画済み配信と、主音声・副音声を同時に配る複数音声レンディションでは `#EXT-X-PART` / `#EXT-X-PRELOAD-HINT` を出さない — Safari のネイティブ HLS が「再生位置のセグメント + ライブ端のパート」を同時に取りに行き、録画では `lastServedSeq` が端へ跳んでエンコード抑制が無効化され (9.8 分の録画を 28 秒でエンコード)、複数音声では音声レンディションを先頭セグメントまでしか取得せず再生が 0.45 秒で止まる
- **複数音声 HLS のマスタープレイリストには `CODECS` を必ず書く** (init の stsd から `Mp4CodecUtil` で読む)。無いと Safari のネイティブ HLS は `video.audioTracks` に 2 本見えていても再生位置が進まない
- **in-memory HLS の字幕 (`emsg`) は必ず version 1**。version 0 だと hls.js が `scheme_id_uri` を読み違え、字幕が一切出ない
- **音声トラック切替は cmd のプレースホルダ経由**。通常の TS は `%DUALMONOMODE%` / `%AUDIOMAP%` を使い、**デュアルモノラル (二か国語) の副音声は `-map` では選べない** — `-dual_mono_mode sub` を使う。encoded tsreplace の `original-hevc` だけは、`VideoUtil.getAudioTracks()` が AAC を main/sub へ実デコードして同じ ES を dual-mono と判定した場合に `filter_complex` の `asplit + pan` で `audio0` / `audio1` へ分ける。EPG の componentType 0x02 や AAC の `channels=2` / `stereo` だけを証拠にしない。通常ステレオ・判定不能は主音声 1 本を保つ。音声 ES 2本は `-map 0:v:0 -map 0:a:0 -map 0:a:1`。filter_complex の枝へ `%AUDIOFILTER%` / `-af` を併用しない。手書き cmd (直書き) では切り替わらない
- **`config.tsreadex` を設定すると生成 cmd の前段へ tsreadex が入る** (`-a 13 -b 7`)。**`-b 5` にしない** — EPG が二か国語でも実際の AAC がデュアルモノラルでない局があり、副音声が完全な無音になる (実測 -91.0dB)。**tsreadex 経由では副音声は `-dual_mono_mode sub` では選べず `-map 0:a:1`** (`AudioTrackUtil` が cmd の `%TSREADEX%` の有無で切り替える)。**`audioTrack=all`** は tsreadex 正規化済み、または実音声 ES が 2 本以上のときに主音声・副音声の両方の ES (`-map 0:a:0 -map 0:a:1`) を同時に配信する (m2tsll でクライアント (mpegts.js) が再接続無しに `switchPrimaryAudio()`/`switchSecondaryAudio()` で切り替えるための経路。`PlaybackProfile.embeddedAudioSwitch` が有効性を示す)。実音声 ES が 1 本または取得不明のときは `main` と同じ扱いにする
- **cmd を生成するコードに `-dual_mono_mode main` を直書きしない** (`StreamProfileManageModel.buildCmd()` / `Live|RecordedCommandBuilder`)。置換対象が消え、API が `audioTrack` を受け取っていても黙って主音声のまま再生される。**`-map 0` を使う container (hls) には `%AUDIOMAP%` を入れない** (ES が二重に出力される)
- **rigaya 系 (QSVEncC 等) の後段 ffmpeg から `-c:v copy` を落とさない**。無いと ffmpeg が出力コンテナの既定コーデック (mpegts なら MPEG-2) で**エンコード済みの映像を再エンコードし直す**。実測 (本番のライブ m2tsll): QSVEncC は HEVC で出しているのに配信 TS が `mpeg2video` になり、mpegts.js が映像を demux できず**音声だけ再生される**状態になっていた (`-c:v copy` は `StreamProfileManageModel.buildRigayaCmd()` の m2tsll の枝だけ抜けていた)
- **tsreadex の前段は `fillGeneratedCmd()` 側だけで付ける**。`buildRigayaCmd()` でも付けると `tsreadex | tsreadex | エンコーダ | ffmpeg` と二重になる (本番の実 cmd で発生していた)
- **複数音声 ES を map する録画配信は probe を広げる**。放送の副音声 ES は番組の途中から現れることがあり (実測: 主音声の 20MB あと)、既定の `-probesize 5000000` だと 2 本目が入力に無い扱いになって `-map "0:a:1?"` が**黙って無視され主音声だけが配信される**。`AudioTrackUtil.ensureMultiAudioProbe()` が `0:a:1` を含む cmd の probe を 50MB / 30 秒まで引き上げる (録画配信で実 ES 2 本以上かつ tsreadex 未経由のときだけ。ライブは初期遅延になるので対象外)
- **tsreadex 未経由の複数音声 ES も音声切替対象**。録画は `VideoUtil.getAudioTracks()`、ライブは番組情報 `audios[]` から実 ES 数を求め、2 本以上なら `sub` を `-map "0:a:1?"` で選ぶ。1 本または取得失敗時は従来の `-dual_mono_mode sub` へ戻す。複数 ES の map は主音声を先にし、HLS の `audio0` / `audio1` と役割を一致させる。in-memory HLS の複数レンディションと `embeddedAudioSwitch.hls=true` の条件も同じにする。rigaya の HLS/MP4 cmd は `%AUDIOMAP%` が映像 map を含むため、別の `-map 0:v:0` を出さない
- **m2tsll は `-map 0` / `-map "0:d?"` (data ストリームの一括 map) を使わない**。相乗りサービスの文字スーパー (PID 0x138、PTS 無しの `bin_data`) が拾われると mpegts muxer がインターリーブ待ちで数フレームだけ書いて完全停止する (実測: ffmpeg 9.0.1、mux 済み frame=5 で固着)。tsreadex 経由は `%AUDIOMAP% -map "0:s?"`、無しは `-map 0:v:0 -map 0:a -map "0:s?" -map "0:i:0x1ffe?"` (ID3 の PID 0x1FFE だけを個別に拾う) にする (`StreamProfileManageModel.buildCmd()`)。ディスク HLS は同じ対処をしていない (未確認のため現状維持)
- **M2TS-LL の生成 cmd は `-probesize 500000` を使い、`-flags low_delay` を付けない**。短周期で届く PAT/PMT の probe 待ちを短くし、放送波 MPEG-2 / インターレースで表示タイミングを不安定にする `-flags low_delay` は使わない。自動生成経路は `LiveCommandBuilder` / `StreamProfileManageModel`、手書き cmd は利用者設定を尊重する
...
- **`mpegts.js` は tsukumijima フォークをコミット SHA で固定**。本家 npm 版は Safari で音声タイムスタンプのギャップ補完が無効化されており、Safari 26.5 以降で再生が止まる。build 時は `client/mpegtsAdtsPatch.js` の置換 (ADTS 偽同期対策と、TSDemuxer が生成時に `config.preferSecondaryAudio` を読む副音声の初期指定) を `client/vite.config.ts` から適用し、依存更新時は置換元が1箇所存在することを確認する。見つからない場合は build を失敗させる
...
- **EPG の「デュアルモノラル」や冒頭の PMT の音声 ES 数を、実際の音声構成の証拠にしない**。EPG が二か国語でも実 AAC はふつうのステレオの録画があり (本番で `-dual_mono_mode main/sub` のデコード結果が完全一致)、左右に割ると主音声がモノラルになる。CM など一部だけ音声 ES が 2 本の録画もあり、必須の `-map 0:a:1` だと ES が無い位置から再生を始めたとき ffmpeg が出力を開けない。判定はファイル内の複数位置の実デコードで行い、map は optional にする (`OriginalHevcUtil.probeAacDualMono()` / `probeSecondAudioStreamPresent()`)
- **オンラインの WebKit へ 10bit (Main 10) HEVC を MSE で流さない**。`MediaSource.isTypeSupported()` も `video.canPlayType()` も「対応」と答えるが、iOS / iPadOS / macOS Safari は 10bit HEVC を MSE 経由で実時間デコードできず**コマ送りになる** (実測: iPad で HEVC Main 10 / 1440x1080 / 29.97fps の録画を無変換再生するとコマ送り、同じ録画の HLS (8bit 変換) は滑らか。同素材を PC Chrome で fMP4 へ remux して直接再生すると平均 30.5fps で正常なので、素材・配信・mpegts.js は健全)。判定は `src/util/OriginalHevcClientSupport.ts` の純粋関数に集約し、オンラインの `RecordedStreamingVideo.vue` がこれを通る。**オフライン保存の `original-hevc` はこの 10bit 除外を適用しない**。保存済み Range VOD を実 Safari 26.6.2 で20秒再生すると `currentTime=19.75`、571 frames、dropped 0、輝度変化ありだったため、`OfflineVideoDownloadDialog.vue` / `OfflineHevcVideo.vue` は mpegts.js の HEVC transmux 能力だけを確認する。**ネイティブ HLS 経由の 10bit (4K HDR 等) は WebKit でも再生できる**ので、`hevcMain10` 能力そのものを false にしてはいけない (4K HDR プリセットまで落ちる)
...
- **録画の MP4 / WebM は WebKit (iOS / iPadOS / Safari) へ出さない**。録画のプログレッシブ配信はエンコーダの出力をそのまま流す chunked で `Content-Length` も `Accept-Ranges` も返せず、WebKit は progressive MP4 を Range 要求で読むため `MediaError code 4` で必ず失敗する。**コーデックの問題ではない** (実測: 本番の録画 MP4 の実データを Content-Length + Range 対応で配ると iPad で再生でき、chunked で配ると HEVC も H.264 も同じく失敗する)。判定は `StreamSupportUtil.isProgressiveFileStreamSupported()`。除外は DPlayer の画質一覧 (`VideoContainer`) と配信選択ダイアログ (`RecordedDetailSelectStreamState`) の両方で行い、**全部落ちて選択肢が空になる設定では元へ戻す**
- **DPlayer の画質メニューの高さは「開いた瞬間」に実座標から決める**。設定パネルはコントローラから上へ開くため、プレイヤーより高いパネルは画面の上へはみ出して上の項目が押せなくなる (実測: iPhone 14 Pro 393x660 でプレイヤー高 217px に対しパネル 294px、上端 y=-131、8 件中 3 件が画面外)。上限は `resolveQualityPanelMaxHeight()` が**パネル下端の実座標からビューポート上端まで**で決め、全項目が収まるなら必要な高さ以上には広げない (プレイヤー高だけで抑えると、プレイヤーが画面の下寄りにあって上に余地があるときまで縮めてしまう。実測: 1280x420 でプレイヤー高 356px のとき上限 290px となり、必要な 354px に届かず末尾 2 項目が押せなかった)。**計算は一覧を作った時点とリサイズ時だけでなく、メニューを開いた瞬間にも走らせる** — `.dplayer-setting-box` の `dplayer-setting-box-quality` クラスを `MutationObserver` で見て、付いた直後と `requestAnimationFrame` 後の 2 回測り直す (開く前の位置・項目数で計算した高さのままだと、開いたときの実際のレイアウトに合わない)。
