...
- **DPlayer の画質メニューの高さはビューポートだけで決めない**。設定パネルはコントローラから上へ開くため、プレイヤーより高いパネルは画面の上へはみ出して上の項目が押せなくなる (実測: iPhone 14 Pro 393x660 でプレイヤー高 217px に対しパネル 294px、上端 y=-131、8 件中 3 件が画面外)。上限は `resolveQualityPanelMaxHeight()` がプレイヤー高からも決め、`ResizeObserver` で回転・全画面・「映像を小さくする」にも追従する
