...
- **WebKit へ 10bit (Main 10) HEVC を MSE で流さない**。`MediaSource.isTypeSupported()` も `video.canPlayType()` も「対応」と答えるが、iOS / iPadOS / macOS Safari は 10bit HEVC を MSE 経由で実時間デコードできず**コマ送りになる** (実測: iPad で HEVC Main 10 / 1440x1080 / 29.97fps の録画を無変換再生するとコマ送り、同じ録画の HLS (8bit 変換) は滑らか。同素材を PC Chrome で fMP4 へ remux して直接再生すると平均 30.5fps で正常なので、素材・配信・mpegts.js は健全)。判定は `src/util/OriginalHevcClientSupport.ts` の純粋関数に集約し、`original-hevc` の再生・オフライン保存の両方がこれを通る。**ネイティブ HLS 経由の 10bit (4K HDR 等) は WebKit でも再生できる**ので、`hevcMain10` 能力そのものを false にしてはいけない (4K HDR プリセットまで落ちる)
