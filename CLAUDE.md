...
- **クライアントのクラスフィールドに書いたコールバックの `this` は Vue インスタンスではない**。`vue-facing-decorator` はクラスフィールドの初期値を data 用の一時インスタンスから集めるため、`private xxxCallback = ((): void => { ... }).bind(this)` の `this` は一時インスタンス (**メソッドだけが Vue インスタンスへ束縛される**)。socket.io コールバックの中から `this.watchParam` のようなデータを読むと初期値しか見えず、条件判定が黙って壊れる。判定・処理はメソッドに置き、フィールドのコールバックはそれを呼ぶだけにすること
- **番組表 (`Guide.vue`) のセルは手組み DOM**。データを取り直したら `GuideState.createProgramDoms()` と `Guide.renderProgramDoms()` の**両方**を呼ぶ (`renderProgramDoms()` を呼ばないと画面が古いまま。可視判定の `updateVisible()` もその末尾で走る)
...
- **TS 解析 (`TsInfoAnalyzer`) は既定でファイル中央から読む**。ファイル先頭には前番組の EIT[p/f] が present のまま流れていて、録画開始直後の壊れた TS も混ざるため、先頭を読むと番組名・概要・ジャンルが前番組のものになる (64MB 以上のファイルのみ中央から読む。`TsInfoAnalyzeOption.analyzeFromMiddle: false` で先頭に戻せる)
    - **`firstTdtAt` は「ファイル先頭の放送時刻」という意味を保つ**。中央の TDT/TOT をそのまま使ってはいけない。`resolveFileStartAt()` が「先頭を読み直して得た TDT/TOT (PCR 補正あり)」と「中央の時刻から実測バイトレート分だけ遡った見積もり」を突き合わせ、5 分以上食い違えば先頭が壊れているとみなして見積もりを採る
    - **相乗りサービスの中から本編サービスを選ぶのは `selectServiceId()`**。全サービス録画の TS にはワンセグ・サブチャンネル・データ放送が同居しており、SDT からは複数の放送局名が取れる。優先順は ①service_type の格 (`0x01`/`0xAD` > `ChannelUtil.isMediaService()` > ワンセグ・データ放送) ②PID ごとのパケット数 (実際に流れている量) ③EIT[p/f] の有無 ④service_id 昇順。EIT[p/f] は「最初に見つかったもの」で確定させず、サービスごとに候補を保持して読み終えてから選ぶ
    - サービス選択がパケット数の偏りを見るため、**最低 20000 パケット読むまで早期打ち切りしない**
    - **番組情報の上書きは明示的な再解析のときだけ**。`analyzeTsInfo(videoFileId, { overwriteProgramInfo: true })` を渡すと `applyProgramInfo()` が概要・詳細・ジャンル・映像音声情報を TS の内容で置き換える (TS 側の組数が減ったジャンルは `null` で消す)。渡すのは全件強制再解析 (`mode: 'all'` / `POST /api/videos/tsinfo/reanalyze`) と録画 1 件の再解析だけで、取り込み・アップロード直後と「未解析のみ」の一括解析は従来どおり**空の項目を補うだけ**。**番組名 (`recorded.name`) はどちらでも上書きしない**
