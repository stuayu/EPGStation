...
| 音声 ES が 1 本 (デュアルモノラル) か 2 本に分かれているか | 副音声の選び方 (`-dual_mono_mode sub` か `-map 0:a:1`) と、ffmpeg の probe 量が足りるかが変わる |

**報告に書かれていない項目は推測せず、コメントで尋ねる。**

## 手元で再現できたら、報告の症状とどこまで同じかを切り分ける

同じ Issue でも**別々の原因が重なっている**ことがある (#31 は実際にそうだった)。

- 手元で再現した不具合は実測で直し、**数値を修正前後で出す**
- 報告者だけで起きている分は、再現していないと明記して open のままにする
- 「たぶんこれも同じ原因」で一緒に close しない
